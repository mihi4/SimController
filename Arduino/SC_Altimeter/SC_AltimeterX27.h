...
unsigned long compass_oldVal =0;
...
void AltimeterX27_Zeroize(bool full)
{
  ...
  if (zero)
    {if (debugmode) sendReadBackString("Compass zeroize success");}
  else
    {if (debugmode) sendReadBackString("Compass zeroize error");}
...
void UpdateAltimeterX27(byte p)
{
  //read target
  ...
  unsigned long compass_newVal = longVal;
  
	/*char buffer[30];
	sendReadBackString("AltimeterValue");
  ...
  if (compass_newVal!=compass_oldVal)
  {
    ...
    if ((int)(target-compass.currentStep)>COMPASSSTEPS1REV/2)                
    {
      ...
      {
        if (COMPASSINVERT)
          {sendReadBackString("Compass transition forward");}
        else
          {sendReadBackString("Compass transition backward");}
      }      
    }
