...
RotEnc rotEnc[]=
{
  ...
  {  2,    3,       0,         1,     "1",   "2",   "3",   "4",  0,  1,  false}
  /*{  2,    4,       5,         1,     "1",   "2",   "3",   "4",  0,  1,  false}
 ,{  3,    6,       7,         2,     "1",   "2",   "3",   "4",  0,  1,  false}*/
};
      ...
      if (rotEnc[x].lastReportedPos < rotEnc[x].encoderPos)
      {
		  if (configmode) {
			  if (pneuActivated()) {
				...
				increaseAltimeterOffset(); 
			  }
		  } else {
			 sendReadBackString("ERROR in encoder check"); // SendMessage(rotEnc[x].commandL,3);
		  }
      ...
      else
      {
		  if (configmode) {
			  if (pneuActivated()) {
				...
				decreaseAltimeterOffset();
			  }
		  } else {
			  sendReadBackString("ERROR in encoder check"); // SendMessage(rotEnc[x].commandR,3);
		  }
        /*if (digitalRead(rotEnc[x].pinShift)==HIGH)
          {
